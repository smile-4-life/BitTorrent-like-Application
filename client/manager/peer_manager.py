...
class PeerManager:
    ...
    def get_peer_with_sock(self, sock):
        found_peers = [peer for peer in self.active_peers if peer.sock == sock]
        return found_peers[0] if found_peers else None
             
    
    # Bitfield + Piece Status
    ...
    def get_peers_with_piece(self, index):
        with self.active_peers_lock:
            return [peer for peer in self.active_peers if peer.index_bitfield.get(index) == 1]
    ...
    def set_peer_choking(self, peer_id):
        ...
        if not peer:
            logging.warning(f"Peer {peer_id} not found to set choking.")
            return
        with peer.status_lock:
    ...
    def add_inflight_request(self, peer):
        with self.inflight_request_lock:
            if peer not in self.inflight_request:
                self.inflight_request.append(peer)
